...
const useVerifyOtp = (): EmailVerificationFunction => {
  ...
  const onVerifyOtp = useCallback(async () => {
    try {
      ...
      const response: any = await verifyOtp({
        postUrl: endpoints.forgotPassword.verifyOtp, // Endpoint to send OTP verification request
      ...
      if (apiResponse.responseCode === "00") {
        ...
        router.navigate("/(forgotPassword)/createPassword"); // Navigate to the create password page
      } else {
...
export default useVerifyOtp; // Export the custom hook for use in other components
