...
const renderedItem = ({ item }: { item: Beneficiary }) => (
    ...
    <TouchableOpacity className="flex-row items-center">
      ...
      <View className="ml-3">
        <View className="flex-row items-center mb-2">
          ...
          <View className="rounded-full py-1 px-3 ml-2 flex-row items-center bg-[#F0F0F0] dark:bg-[#5C5C5C]">
            <Text className="text-sm dark:text-white">
        ...
        <Text className="text-sm dark:text-white truncate max-w-[10rem] break-words">
          {maskAccountNumber(item.accountNumber || item.walletAccountName)} |{" "}
          {item.bankName ?? item.walletType}
        </Text>
      </View>
    </TouchableOpacity>
    {/* {deleteIndex === index && deleteDataResponse?.isLoading ? (
          <ActivityIndicator />
        ) : ( */}
    <TouchableOpacity>
...
const BeneficiaryList = () => {
  ...
  return (
    <View className="p-5 flex-1">
      <Text className="font-InterRegular text-sm dark:text-white">My beneficiaries</Text>
      <FlatList
        data={filteredData}
